...
def image_transform(image, prompt, coordinates):
    
    if coordinates == []:  # 좌표가 없다면 이미지 전체 변환(pix2pix)
        print("instruct_pix2pix Start")
        transform_pillow = instruct_pix2pix(image, prompt)[0]
    else: # 좌표가 있다면 특정 영역만 inpaint
        coordinates = np.array(coordinates)
        if coordinates.shape[1] == 1:  # coordinates(freedraw로 만들어짐) 자체가 마스크 이므로 inpaint만 수행
            print("inpaint Start")
            mask = Image.fromarray(st.session_state["mask"])
            transform_pillow = sd_inpaint(image, mask, prompt)
            
        else:
            print("sam_inpaint Start")
            transform_pillow = sam_inpaint(image, prompt, coordinates)
    
    return transform_pillow

def object_erase(image, coordinates):
    
    coordinates = np.array(coordinates)
    if coordinates.shape[1] == 1:  # 마스킹 되었을 때 (freedraw)
        print("erase Start")
        image = np.array(image)
        mask = st.session_state["mask"]
    else:  # 마스킹 해야할 때 (point, box)
        print("sam_erase Start")
        image, mask = light_hqsam(image, coordinates)
...
class ImageTransformCheckInput(BaseModel):
    ...
    coordinates: List[List[float]] = Field(..., description="coordinates for segment object")

class ObjectEraseCheckInput(BaseModel):
    coordinates: List[List[float]] = Field(..., description="coordinates for segment object")
...
class ImageTransformTool(BaseTool):
    ...
    description = """
    Please use this tool when you want to change the image style or replace specific objects with something else.
    """
    
    def _run(self, prompt: str, coordinates: List[List[float]]):

        image = st.session_state["inference_image"][st.session_state["image_state"]]
        transform_pillow = image_transform(image, prompt, coordinates)
...
class ObjectEraseTool(BaseTool):  
    ...
    def _run(self, coordinates: List[List[float]]):
        image = st.session_state["inference_image"][st.session_state["image_state"]]
        transform_pillow = object_erase(image, coordinates)
        st.session_state["inference_image"].insert(st.session_state["image_state"]+1, transform_pillow)
        st.session_state["image_state"] += 1
    ...
    args_schema: Optional[Type[BaseModel]] = ObjectEraseCheckInput
